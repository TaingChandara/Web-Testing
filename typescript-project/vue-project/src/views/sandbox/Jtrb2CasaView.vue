...
import {
    SandboxService,
    type BankListProp,
    type InitCasaTrxResponse,
    type TrxAmountResponse,
    type TransferCasaResponse,
...
const bankListOptions: Ref<BankListProp[]> = ref([]);
...
onMounted(async () => {
    try {
        loading.bankList = true;
        bankListOptions.value = await SandboxService.getBankList();
    } catch (err) {
        evs.captureError(err);
    } finally {
        loading.bankList = false;
    }
});

const trxInfo = reactive<Partial<InitCasaTrxResponse>>({});
const setInitTrx = (data?: InitCasaTrxResponse) => {
...
const fundTransfer = reactive<Partial<TransferCasaResponse>>({});
const setFundTransfer = (data?: TransferCasaResponse) => {
...
const handleInitTrx = async () => {
    ...
    if (!formData.participantCode) return evs.captureError('Beneficiary Bank Name is required');
    ...
    try {
        ...
        const trxData = await SandboxService.initCasaTrx({
...
const handleSubmit = async () => {
    ...
    try {
        // reset data
        setFundTransfer();
        const transferData = await SandboxService.transferCasa({
...
// dialog account =================================
const setAccount = (data: TestAccountProp) => {
...
const changeParticipantCode = () => {
    setInitTrx();
    setTransferPayload();
    resetAmount();
    formData.accountNumber = null;
    formData.transactionCurrency = null;
    v$.value.accountNumber.$reset();
    v$.value.transactionCurrency.$reset();
};
const changeTrxCCY = () => {
    setTransferPayload();
    resetAmount();
...
<template>
                ...
                <form class="p-fluid mt-5" @submit.prevent="dialog.transfer = true">
                    ...
                    <div class="field mt-4">
                        <div class="p-float-label">
                            <PDropdown
                                id="participantCode"
                                v-model="v$.participantCode.$model"
                                :options="bankListOptions"
                                optionLabel="BankName"
                                optionValue="ParticipantCode"
                                :class="{ 'p-invalid': v$.participantCode.$invalid && v$.participantCode.$dirty }"
                                :disabled="loading.bankList"
                                :loading="loading.bankList"
                                :showClear="true"
                                @change="changeParticipantCode"
                            />
                            <label for="participantCode">Beneficiary Bank Name*</label>
                        </div>
                        <span v-if="v$.participantCode.$silentErrors && v$.participantCode.$dirty">
                            <span v-for="(error, index) of v$.participantCode.$silentErrors" :key="index">
                                <small class="p-error">{{
                                    String(error.$message).replace('Value', 'Beneficiary Bank Name')
                                }}</small>
                            </span>
                        </span>
                    </div>
                    <div class="field mt-5">
                        <div class="p-float-label" :class="{ 'p-input-icon-right': loading.initTrx }">
                            ...
                            <PInputText
                                ...
                                :disabled="loading.initTrx || !v$.participantCode.$model"
                                @blur="handleInitTrx"
                        ...
                        <span v-if="v$.accountNumber.$silentErrors && v$.accountNumber.$dirty">
                            <span v-for="(error, index) of v$.accountNumber.$silentErrors" :key="index">
                                <small class="p-error">{{
                                    String(error.$message).replace('Value', 'Beneficiary Account Number')
                                }}</small>
                            </span>
                        </span>
                    </div>
    ...
    >
        ...
        <div>
            <div class="font-bold text-sm text-gray-300">Beneficiary Bank Name</div>
            <div class="font-bold text-sm mt-2 mb-4">{{ transferPayload.creditor?.BankName }}</div>
            <div class="font-bold text-sm text-gray-300">Beneficiary Account Number</div>
            <div class="font-bold text-sm mt-2 mb-4">{{ transferPayload.creditor?.AccountNumber }}</div>
            <div class="font-bold text-sm text-gray-300">Account Name</div>
    ...
    <PDialog v-model:visible="dialog.success" :closable="false" :modal="true" :style="{ width: '480px' }">
        ...
        <div class="text-center">
            ...
            <div class="mt-6">
                ...
                <div class="font-bold text-sm text-gray-300">Beneficiary Bank</div>
                <div class="font-bold text-sm mt-2 mb-4">{{ fundTransfer.creditor?.BankName }}</div>
                <div class="font-bold text-sm text-gray-300">Account Number</div>
